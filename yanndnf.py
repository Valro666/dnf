...
class yanndnf:
    ...
    def update_neuron(self, x):
        ...
        self.potentials[x] = self.a.runge(self.lateral[x],self.temps,self.tau)+self.input[x]-self.potentials[x]
...
def updatefig(*args):
    ...
    plt.subplot(2, 2, 2)
    im2 = plt.imshow(dnf.input, cmap='hot', interpolation='nearest', animated=True)
    plt.title("input")
    plt.subplot(2, 2, 3)
    im2 = plt.imshow(dnf.kernel, cmap='hot', interpolation='nearest', animated=True)
    plt.title("kernel")
    plt.subplot(2, 2, 4)
    im2 = plt.imshow(dnf.lateral, cmap='hot', interpolation='nearest', animated=True)
    plt.title("lateral")
